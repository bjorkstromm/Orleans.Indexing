...
namespace Orleans.Indexing
{
    ...
    internal class IndexWorkflowQueueBase : IIndexWorkflowQueue
    {
        ...
        private IndexWorkflowRecordNode? AddPunctuationAt(int batchSize)
        {
            ...
            if (punctuationHead?.IsPunctuation ?? false) throw new WorkflowIndexException("The element at the head of workflow queue cannot be a punctuation.");
        ...
        private List<IndexWorkflowRecord> RemoveFromQueueUntilPunctuation(IndexWorkflowRecordNode? from)
        {
            ...
            if (from is not null && !from.IsPunctuation && from.WorkflowRecord is not null)
            {
                workflowRecords.Add(from.WorkflowRecord);
            }

            IndexWorkflowRecordNode? tmp = from?.Next;
            while (tmp is not null && !tmp.IsPunctuation)
            {
                if (tmp.WorkflowRecord is not null)
                {
        ...
        private async Task PersistState()
        {
            ...
            using (await _writeLock.LockAsync())
            {
                ...
                if (_pendingWriteRequests.Contains(writeRequestId))
                {
                    ...
                    try
                    {
                        this.queueState.ETag = ANY_ETAG;
                        await (StorageProvider?.WriteStateAsync(_grainTypeName, _lazyParent.Value.GrainId, this.queueState) ?? Task.CompletedTask);
                    }
                    finally
                    {
                        if (this.queueState.ETag == ANY_ETAG)
                        {
        ...
        public Task<Immutable<IndexWorkflowRecordNode>> GiveMoreWorkflowsOrSetAsIdle()
        {
            ...
            else
            {
                ...
                return Task.FromResult(AddPunctuationAt(BATCH_SIZE)?.AsImmutable() ?? EmptyIndexWorkflowRecordNode);
            }
        }
        ...
        public async Task<Immutable<List<IndexWorkflowRecord>>> GetRemainingWorkflowsIn(HashSet<Guid> activeWorkflowsSet)
        {
            ...
            {
                if (current.WorkflowRecord is not null
                    && activeWorkflowsSet.Contains(current.WorkflowRecord.WorkflowId))
                {
...
#region STATIC HELPER FUNCTIONS
        ...
        {
            int queueSeqNum = PositiveHash(grainHashCode, siloIndexManager.NumWorkflowQueuesPerInterface);
        ...
        private static int PositiveHash(int hash, int hashRange)
        {
